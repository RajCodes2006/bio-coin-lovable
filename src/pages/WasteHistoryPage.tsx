...
import { wasteReports, WasteReport } from "@/lib/mock-data";
import { ArrowLeft, Plus, Trash2, Eye, Filter } from "lucide-react";
...
const WasteHistoryPage: React.FC = () => {
  ...
  const [reports] = useState<WasteReport[]>(wasteReports);
              ...
              <tbody>
                {filtered.map((r) => (
                  <tr key={r.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm font-semibold text-success">{r.coins > 0 ? `+${r.coins}` : "-"}</td>
                    <td className="px-4 py-3 flex gap-2">
                      <button className="text-primary hover:text-primary/80"><Eye size={16} /></button>
